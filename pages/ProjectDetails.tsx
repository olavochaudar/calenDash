...
import { 
    ArrowLeft, Save, Tag, MapPin, AlignLeft, CheckSquare, 
    Plus, Trash2, Image as ImageIcon, Upload, X, MoreVertical, 
    ArrowDownUp, Filter
} from 'lucide-react';
...
export const ProjectDetails: React.FC = () => {
  ...
  const [taskSort, setTaskSort] = useState<'newest' | 'oldest' | 'az'>('newest');
  ...
  const handleSave = async () => {
    ...
    setEvent(prev => prev ? ({ ...prev, brief: description, content: notes, status, tasks, attachments }) : null);
    setSaving(false);
  };
  ...
  if (loading) return <div className="flex items-center justify-center h-[50vh] text-gray-500 animate-pulse">Carregando workspace...</div>;
  if (!event) return <div className="text-center text-gray-500 mt-20">Evento não encontrado.</div>;
    ...
    <div className="space-y-6 pb-20">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
              ...
              <select 
                ...
                className={`bg-gray-900 border border-gray-700 text-sm rounded-lg p-2.5 outline-none font-medium w-full md:w-auto
                    ${status === 'scheduled' ? 'text-blue-400' : status === 'completed' ? 'text-green-400' : 'text-red-400'}
                `}
              >
              ...
              <Button onClick={handleSave} isLoading={saving} className="whitespace-nowrap">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="flex justify-between items-start mb-6">
                      ...
                      <div className="flex flex-col items-end">
                          <div className="w-14 h-14 bg-gray-900 rounded-xl flex flex-col items-center justify-center border border-gray-700 text-gray-300 shadow-lg">
                               <span className="text-[10px] font-bold uppercase">{new Date(event.created_at).toLocaleString('default', { month: 'short' })}</span>
                               <span className="text-xl font-bold text-white">{new Date(event.created_at).getDate()}</span>
                          </div>
                      </div>
                  </div>
              ...
              <div className="bg-brand-surface border border-gray-800 rounded-2xl p-6">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                      <h3 className="text-xl font-bold text-white flex items-center gap-2">
                          <CheckSquare size={20} className="text-indigo-500" /> O Que Será Feito
                      </h3>
                      ...
                      <div className="flex items-center gap-2">
                        <div className="bg-gray-900 border border-gray-700 rounded-lg p-1 flex">
                            <button 
                                ...
                                className={`px-2 py-1 text-xs font-medium rounded transition-colors ${taskFilter === 'all' ? 'bg-gray-800 text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'}`}
                            >
                                Todas
                            </button>
                            <button 
                                onClick={() => setTaskFilter('pending')}
                                className={`px-2 py-1 text-xs font-medium rounded transition-colors ${taskFilter === 'pending' ? 'bg-gray-800 text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'}`}
                            >
                                Pendentes
                            </button>
                            <button 
                                onClick={() => setTaskFilter('completed')}
                                className={`px-2 py-1 text-xs font-medium rounded transition-colors ${taskFilter === 'completed' ? 'bg-gray-800 text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'}`}
                            >
                                Feitas
                            </button>
                        </div>
                          ...
                          >
                              <button 
                                ...
                                className={`flex-shrink-0 w-6 h-6 rounded-md border flex items-center justify-center transition-colors ${
                                    task.completed ? 'bg-green-500 border-green-500 text-white' : 'border-gray-600 hover:border-indigo-500'
                                }`}
                              >
          ...
          <div className="space-y-6">
              ...
              <div className="bg-brand-surface border border-gray-800 rounded-2xl p-6">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                      {attachments.map((img, idx) => (
                          <div key={idx} className="aspect-square rounded-xl relative group overflow-hidden border border-gray-800">
                              <img src={img} alt={`attachment-${idx}`} className="w-full h-full object-cover" />
                              <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                  <button onClick={() => window.open(img, '_blank')} className="p-1.5 bg-gray-700 text-white rounded-full hover:bg-indigo-500">
                                      <MoreVertical size={14} />
                                  </button>
                                  <button onClick={() => removeImage(idx)} className="p-1.5 bg-red-500/80 text-white rounded-full hover:bg-red-600">
                                      <X size={14} />
                                  </button>
                              </div>
                          </div>
              ...
              {/* Quick Info */}
              <div className="bg-brand-surface border border-gray-800 rounded-2xl p-6">
                  <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4">Detalhes</h3>
                  <div className="space-y-4">
                      ...
                      <div className="flex justify-between items-center text-sm">
                          ...
                          <span className="text-gray-300">{new Date(event.created_at).toLocaleDateString()}</span>
                      </div>
                  </div>
              </div>
