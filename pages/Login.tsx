...
export const Login: React.FC<{ onLogin: () => void }> = ({ onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Pequeno delay para efeito visual de carregamento
    ...
    if (error) {
      setError('E-mail ou senha incorretos.'); // Mensagem mais amigável que error.message original
      setLoading(false);
    } else if (data) {
      onLogin();
      navigate('/');
      ...
      <div className='hidden lg:flex w-1/2 relative bg-black overflow-hidden flex-col justify-between p-12 border-r border-gray-900'>
        ...
        <div className='relative z-10 max-w-lg'>
          <h2 className='text-4xl font-bold text-white mb-6 leading-tight'>
            Gestão de <span className='text-[#3B82F6]'>agendas e equipes</span>{' '}
            unificada.
          </h2>
        ...
        <div className='relative z-10 bg-gray-900/50 backdrop-blur-md p-6 rounded-2xl border border-gray-800'>
          <p className='text-gray-300 italic mb-4'>
            "O CalenDash trouxe visibilidade para o que cada membro da equipe
            está fazendo, eliminando reuniões de status desnecessárias."
          </p>
      ...
      <div className='w-full lg:w-1/2 flex flex-col justify-center items-center p-6 md:p-12 bg-[#09090b]'>
        <div className='w-full max-w-md space-y-8'>
          ...
          <p className='mt-8 text-center text-sm text-gray-500'>
            ...
            <Link
              to='/register'
              className='font-bold text-[#3B82F6] hover:text-blue-400 hover:underline transition-colors'
            >
