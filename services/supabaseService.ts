...
import { User, Project, Client, KpiData, ChartData, Template, Product } from '../types';
...
const MOCK_EVENTS: Project[] = [
  { 
    ...
    priority: 'high',
    tasks: [
        { id: 't1', text: 'Preparar apresentação de slides', completed: true, created_at: '2023-10-20T10:00:00Z' },
        { id: 't2', text: 'Confirmar presença dos stakeholders', completed: false, created_at: '2023-10-21T14:30:00Z' },
        { id: 't3', text: 'Reservar sala de reunião', completed: true, created_at: '2023-10-22T09:15:00Z' }
    ],
    attachments: [
  ...
  { 
    ...
    priority: 'medium',
    tasks: [
        { id: 't1', text: 'Revisar copy dos posts', completed: false, created_at: '2023-10-24T11:00:00Z' },
        { id: 't2', text: 'Aprovar design das artes', completed: false, created_at: '2023-10-24T11:05:00Z' }
    ],
    attachments: []
  },
  { id: '3', name: 'Call Mensal de Resultados', type: 'Recorrente', status: 'completed', created_at: '2023-10-20', brief: 'Apresentação de KPIs.', content: 'Slide deck em anexo.', priority: 'high' },
  { id: '4', name: 'Dentista', type: 'Pessoal', status: 'scheduled', created_at: '2023-10-28', brief: 'Dr. Silva - 15h', content: '', priority: 'low' },
];
...
export const MOCK_TEMPLATES: Template[] = [
  { 
    ...
    fullDesc: 'Este modelo é perfeito para gestores de mídias sociais. Ele inclui visualizações semanais e mensais, campos para status de aprovação, links de assets e legendas. Otimize seu fluxo de publicação e nunca mais perca uma data comemorativa.',
    features: ['Planejador de Feed', 'Status de Aprovação', 'Banco de Hashtags', 'Métricas de Performance'],
    popularity: 98
  },
  { 
    id: 2, 
    ...
    category: 'Negócios', 
    image_url: 'https://images.unsplash.com/photo-1531403009284-440f080d1e12?q=80&w=1000&auto=format&fit=crop',
    shortDesc: 'Sprints, dailies e entregas para times de tech.',
    fullDesc: 'Baseado na metodologia Scrum, este calendário foca em ciclos de 2 semanas (Sprints). Organize suas dailies, reviews e retrospectives. Ideal para manter o time alinhado e visualizar o progresso das tarefas.',
    features: ['Ciclos de Sprint', 'Datas de Release', 'Meeting Planner', 'Blockers Log'],
    popularity: 85
  },
  { 
    id: 3, 
    ...
    fullDesc: 'Acompanhe sua evolução física com este calendário de treinos. Defina grupos musculares por dia, dias de descanso ativo e registre suas cargas. A consistência é a chave para o resultado.',
    features: ['Divisão de Treino ABC', 'Registro de Cardio', 'Controle de Peso', 'Metas Mensais'],
    popularity: 72
  },
  { 
    id: 4, 
    ...
    fullDesc: 'Maximize seu aprendizado distribuindo as matérias ao longo da semana. Utilize a técnica de pomodoro integrada e reserve tempo para revisões espaçadas.',
    features: ['Grade Horária', 'Revisão Espaçada', 'Datas de Provas', 'Progresso de Matéria'],
    popularity: 90
  }
];
...
export const supabaseService = {
  auth: {
    ...
    updateUser: async (updates: Partial<User>) => {
       ...
       const current = JSON.parse(localStorage.getItem('calendash_session') || '{}');
  ...
  from: (table: string) => {
    return {
      select: async () => {
        ...
        if (table === 'projects') return { data: [...MOCK_EVENTS], error: null };
        if (table === 'templates') return { data: [...MOCK_TEMPLATES], error: null };
        if (table === 'clients') {
        ...
        if (table === 'kpis') {
             const kpis: KpiData[] = [
                { label: 'Eventos Agendados', value: '42', change: 15, trend: 'up' },
      ...
      delete: async () => { return { error: null }; },
